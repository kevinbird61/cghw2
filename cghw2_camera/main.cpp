...
// Function prototypes - For camera
...
static void render(const glm::mat4 rev_earth_to_sun,const glm::mat4 rev_moon_to_earth,const glm::mat4 rot_sun,const glm::mat4 rot_earth, const glm::mat4 rot_moon)
{
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
	glm::mat4 view;
    view = camera.GetViewMatrix();
    float angle_cam = 20.0f;
    glm::mat4 projection = glm::perspective(camera.Zoom, (GLfloat)WIDTH / (GLfloat)HEIGHT, 0.1f, 100.0f)*glm::lookAt(glm::vec3(angle_cam), glm::vec3(), glm::vec3(0, 0, 1));
	...
	for(int i=0;i<objects.size();i++){
		...
		switch (i){
			case 0:
				// Sun
				setUniformMat4(objects[i].program, "projection", projection);
				setUniformMat4(objects[i].program , "vp" , rot_sun*view);	
				setUniformMat4(objects[i].program, "model", objects[i].model);	
				break;
			case 1:
				// Earth
				setUniformMat4(objects[i].program, "projection", glm::perspective(glm::radians(45.0f), 640.0f/480, 1.0f, 100.f)*glm::lookAt(glm::vec3(angle_cam), glm::vec3(), glm::vec3(0, 0, 1)));
				setUniformMat4(objects[i].program , "vp" ,rev_earth_to_sun*rot_earth*view);	
				setUniformMat4(objects[i].program, "model", objects[i].model);	
				break;
			case 2:
				// Moon
				setUniformMat4(objects[i].program, "projection", glm::perspective(glm::radians(45.0f), 640.0f/480, 1.0f, 100.f)*glm::lookAt(glm::vec3(angle_cam), glm::vec3(), glm::vec3(0, 0, 1)));
				setUniformMat4(objects[i].program , "vp" , rev_earth_to_sun*rot_earth*rev_moon_to_earth*rot_moon*view);	
				setUniformMat4(objects[i].program, "model", objects[i].model*glm::scale(glm::mat4(1.0f), glm::vec3(0.5f)) );
				break;
		}
...
int main(int argc, char *argv[])
{
	...
	int width = WIDTH;
	int height = HEIGHT;
	window = glfwCreateWindow(width, height, "CGHW3 - Shader", NULL, NULL);
	if (!window)
	{
		glfwTerminate();
		return EXIT_FAILURE;
	...
	// For Camera : GLFW Options 
	glfwSetInputMode(window , GLFW_CURSOR , GLFW_CURSOR_DISABLED);
	
	// For Camera : Set the View Port
	...
	GLint objectColorLoc2 = glGetUniformLocation(program2 , "objectColor");
	GLint lightColorLoc2 = glGetUniformLocation(program2 , "lightColor");
	GLint lightPosLoc2 = glGetUniformLocation(program2 , "lightPos");
	GLint viewPosLoc2 = glGetUniformLocation(program2 , "viewPos");
	glUniform3f(objectColorLoc2 , 1.0f , 0.5f , 0.31f);
	...
	GLint objectColorLoc3 = glGetUniformLocation(program3 , "objectColor");
	GLint lightColorLoc3 = glGetUniformLocation(program3 , "lightColor");
	GLint lightPosLoc3 = glGetUniformLocation(program3 , "lightPos");
	GLint viewPosLoc3 = glGetUniformLocation(program3 , "viewPos");
	glUniform3f(objectColorLoc3 , 1.0f , 0.5f , 0.31f);
	...
	objects[moon].model = glm::mat4(1.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		rot =  glm::rotate(glm::mat4(), (GLfloat)glfwGetTime() * 0.1f, glm::vec3(0.0f, 0.0f, 1.0f));
		...
		rev = glm::translate(glm::mat4(),glm::vec3(14.0f*cos(glfwGetTime()),12.0*sin(glfwGetTime()),0.0f));
		rev2 = glm::translate(glm::mat4(1.0f),glm::vec3(5.0f*cos(glfwGetTime()+180),5.0f*sin(glfwGetTime()+180),0.0f));
		
		// Render the planets
		glClearColor(0.1f,0.1f,0.1f,1.0f);
		
		render(rev,rev2,rot,rot2,rot3);
